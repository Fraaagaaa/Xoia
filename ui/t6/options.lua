...
CoD.Options.AddOptionCategories = function ( f42_arg0 )
	...
	if UIExpression.IsInGame() == 0 then
		...
		local f42_local_xoia = f42_local2:addNavButton("XOIA", "xoia")
		...
		if not CoD.isSinglePlayer then
			...
			-- Ocultar corchetes de tu nuevo boton
			f42_local_xoia.brackets:close()
			f42_local_xoia.m_skipAnimation = true
		end
	else
		if CoD.isSinglePlayer then
...
LUI.createMenu.OptionsMenu = function ( f43_arg0 )
	...
	f43_local0:registerEventHandler( "xoia", CoD.Options.OpenStratTesterSettings )
