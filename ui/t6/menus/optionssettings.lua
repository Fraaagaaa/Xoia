...
LUI.createMenu.OptionsSettingsMenu = function (LocalClientIndex)
	...
	-- FIX (tarea: "PAUSED AT ROUND" visible siempre durante la pausa):
	-- mismo bloque ya añadido en OptionsMenu (options.lua), XoiaMenu
	-- (optionsxoia.lua) y OptionsControlsMenu (optionscontrols.lua),
	-- reutilizando el mismo dvar ("xoia_info_round", publicado por
	-- Xoia_ui.gsc::update_info_dvars()) y la misma clave de localizacion
	-- (XOIA_MENU_PAUSED_AT_ROUND). Solo se añade si estamos en partida
	-- (InGame): si este menu se abre desde el menu principal no hay
	-- ronda que mostrar. Centrado, igual que en XoiaMenu.
	if InGame and CoD.isZombie == true then
		local PausedAtRoundText = LUI.UIText.new()

		PausedAtRoundText:setLeftRight( true, true, 0, 0 )
		PausedAtRoundText:setTopBottom( true, false, 40, 40 + CoD.textSize.Default )
		PausedAtRoundText:setFont( CoD.fonts.Default )
		PausedAtRoundText:setAlignment( LUI.Alignment.Center )
		...
		OptionsSettingsWidget:addElement( PausedAtRoundText )
	end
