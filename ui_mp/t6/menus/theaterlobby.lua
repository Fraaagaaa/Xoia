...
CoD.TheaterLobby.CloseAllPopups = function (Popup, ClientInstance)
...
CoD.TheaterLobby.LeaveTheaterLobby = function (TheaterLobby, ClientInstance)
	Engine.SetTheaterFileInfo(false)
	
	Engine.SetDvar("invite_visible", 1)
	Engine.SetGametype(Dvar.ui_gametype:get())
	
	if ClientInstance.name == nil or ClientInstance.name == "confirm_leave_alone" then
...
CoD.TheaterLobby.BackButtonAction = function (TheaterLobby, ClientInstance)
	CoD.Lobby.ConfirmLeaveGameLobby(TheaterLobby, {
		controller = ClientInstance.controller,
		leaveLobbyHandler = CoD.TheaterLobby.LeaveTheaterLobby
	})
end

CoD.TheaterLobby.DemoSortFunc = function (a, b)
...
CoD.TheaterLobby.StartDemoAction = function (Button, ClientInstance)
	if UIExpression.DvarInt(nil, "party_playerCount") > 1 then
		Engine.ExecNow( 0, "xstopallparties\n" )
	end
	
	Engine.Exec(0, "demo_play " .. CoD.TheaterLobby.CurrentDemo.name .. "\n")
end

CoD.TheaterLobby.StartShoutcastDemoAction = function (Button, ClientInstance)
	if UIExpression.DvarInt(nil, "party_playerCount") > 1 then
		Engine.ExecNow( 0, "xstopallparties\n" )
	end
...
CoD.TheaterLobby.ButtonActionFunc = function (Button, ClientInstance)
	CoD.TheaterLobby.BaseMenu:openPopup("DemoListPopup", ClientInstance.controller)
end

CoD.TheaterLobby.DeleteDemoPopupAction = function (Button, ClientInstance)
	local popup = Button:getParent():getParent()
	CoD.TheaterLobby.CloseAllPopups(popup, ClientInstance)
	CoD.TheaterLobby.BaseMenu:openPopup("DeleteFilmPopup", ClientInstance.controller)
end

CoD.TheaterLobby.GetZombieGameTypeName = function (Gametype, Mapname)
...
CoD.TheaterLobby.GetDisplayableMap = function (Mapname, Gametype, Location)
...
CoD.TheaterLobby.GetDisplayableGametype = function (Gametype, Mapname)
	if CoD.isZombie then
		if Gametype == "zclassic" and Mapname ~= "zm_transit" then
			Gametype = "ZMUI_" .. Gametype .. "_" .. Mapname
		else
			Gametype = CoD.TheaterLobby.GetZombieGameTypeName(Gametype)
		end
	else
...
CoD.TheaterLobby.ShowButtonBorderFunc = function (ButtonBorder, ClientInstance)
...
CoD.TheaterLobby.CreateDemoInfoSection = function (TheaterLobbyMenu)
...
CoD.TheaterLobby.CreateDemoListButton = function (TheaterLobbyMenu, f2_arg0, DemoInfo, UIElementTable, ClientInstance)
	...
	MapName:setText(CoD.TheaterLobby.GetDisplayableMap(DemoInfo.map, DemoInfo.gametype, DemoInfo.location))
	...
	ButtonBorder:hide()
	ButtonBorder:registerEventHandler("button_over", CoD.TheaterLobby.ShowButtonBorderFunc)
	ButtonBorder:registerEventHandler("button_up", ButtonBorder.hide)
	DemoListButton:addElement(ButtonBorder)
	return DemoListButton
end

CoD.TheaterLobby.RefreshListFunc = function (TheaterLobbyMenu, DemoList)
	DemoList.verticalList:removeAllChildren()
	DemoList.scrollbar.image:setRGB(CoD.BOIIOrange.r, CoD.BOIIOrange.g, CoD.BOIIOrange.b)
	local DemoListings = Engine.GetDemoList()
	if DemoListings == nil or #DemoListings == 0 then
		return
	end
	table.sort(DemoListings, CoD.TheaterLobby.DemoSortFunc)
	for DemoListingsIndex, DemoInfo in ipairs(DemoListings) do
		local DemoListButton = CoD.TheaterLobby.CreateDemoListButton(TheaterLobbyMenu, DemoListingsIndex, DemoInfo, {
			left = 0,
			top = 0,
		...
		DemoListButton:registerEventHandler("button_action", CoD.TheaterLobby.ButtonActionFunc)
...
CoD.TheaterLobby.DeleteFilmAction = function (Popup, ClientInstance)
	if Engine.DeleteDemoFile(CoD.TheaterLobby.CurrentDemo.index) == true then
		CoD.TheaterLobby.RefreshListFunc(CoD.TheaterLobby.BaseMenu, CoD.TheaterLobby.BaseMenu.demoList)
	end
	
	CoD.TheaterLobby.CloseAllPopups(Popup, ClientInstance)
end

CoD.TheaterLobby.CreateDemoList = function (TheaterLobbyMenu, UIElementTable)
	local DemoList = CoD.ScrollingVerticalList.new(UIElementTable)
	DemoList.id = "DemoList"
	CoD.TheaterLobby.RefreshListFunc(TheaterLobbyMenu, DemoList)
	return DemoList
end
...
LUI.createMenu.DeleteFilmPopup = function (LocalClientIndex)
	...
	CoD.PopupMenus.SetupConfirmPopup(DeleteFilmPopup, Engine.Localize("MENU_DELETE"), Engine.Localize("MENU_DELETE_SEGMENT_CONFIRMATION"), CoD.TheaterLobby.DeleteFilmAction, CoD.TheaterLobby.CloseAllPopups)
...
LUI.createMenu.DemoListPopup = function (LocalClientIndex)
	...
	DemoListPopup:registerEventHandler("closeallpopups", CoD.TheaterLobby.CloseAllPopups)
	...
	DemoListPopup.startFilmButton:registerEventHandler("button_action", CoD.TheaterLobby.StartDemoAction)
	
	if not CoD.isZombie then
		DemoListPopup.startFilmButton = DemoListPopup.buttonList:addButton(Engine.Localize("MPUI_SHOUTCAST_FILM_CAPS"))
		DemoListPopup.startFilmButton:registerEventHandler("button_action", CoD.TheaterLobby.StartShoutcastDemoAction)
	end
	...
	DemoListPopup.deleteFilmButton:registerEventHandler("button_action", CoD.TheaterLobby.DeleteDemoPopupAction)
...
LUI.createMenu.TheaterLobby = function (LocalClientIndex, f1_arg1)
	...
	TheaterLobbyMenu:registerEventHandler("button_prompt_back", CoD.TheaterLobby.BackButtonAction)
	...
	if CoD.isPartyHost() then
		...
		TheaterLobbyMenu.demoList = CoD.TheaterLobby.CreateDemoList(TheaterLobbyMenu, {
			left = CoD.TheaterLobby.DemoList.LeftOffset,
			top = CoD.Menu.TitleHeight + CoD.CoD9Button.Height,
			right = CoD.TheaterLobby.DemoList.ColumnTotalWidth + CoD.TheaterLobby.DemoList.LeftOffset + 10,
			bottom = -CoD.CoD9Button.Height - 10,
			leftAnchor = true,
			topAnchor = true,
			rightAnchor = false,
		...
		CoD.TheaterLobby.CreateDemoInfoSection(TheaterLobbyMenu)
	end
