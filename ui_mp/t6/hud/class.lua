...
-- FIX (pedido por el usuario): mismo patron ya usado en options.lua
-- (CoD.Options.PauseHard) — "pause_hard" es un comando de consola nativo
-- del motor, no necesita GSC.
CoD.Class.PauseHardButtonPressed = function ( f6b_arg0, f6b_arg1 )
	Engine.Exec( f6b_arg1.controller, "pause_hard" )
end
...
CoD.Class.PrepareClassButtonList = function ( f12_arg0, f12_arg1 )
	...
	if CoD.isZombie == true then
		...
		CoD.Class.AddButton( f12_arg1, Engine.Localize( "XOIA_MENU_PAUSE_HARD" ), "xoia_pause_hard" )
	else
...
LUI.createMenu.class = function ( f13_arg0 )
	...
	-- FIX (pedido por el usuario): mismo bloque "PAUSED AT ROUND" ya
	-- añadido en OptionsMenu/XoiaMenu/OptionsSettingsMenu/OptionsControlsMenu.
	-- Reutiliza el mismo dvar ("xoia_info_round", publicado por
	...
	if CoD.isZombie == true then
		...
		f13_local_roundText:setTopBottom( true, false, 40, 40 + CoD.textSize.Default )
		f13_local_roundText:setFont( CoD.fonts.Default )
		f13_local_roundText:setAlignment( LUI.Alignment.Center )

		local currentRound = UIExpression.DvarInt( f13_arg0, "xoia_info_round" )
		if currentRound == nil then currentRound = 0 end

		f13_local_roundText:setText( Engine.Localize( "XOIA_MENU_PAUSED_AT_ROUND" ) .. " " .. tostring( currentRound ) )

		f13_local1:addElement( f13_local_roundText )
	end
	...
	if CoD.isZombie == true then
		...
		f13_local1:registerEventHandler( "xoia_pause_hard", CoD.Class.PauseHardButtonPressed )
	end
